...
// rareté des COMPAGNONS invoqués : mêmes 6 paliers/couleurs que les objets (RARITIES),
...
const ALLY_RARITY_MULT=[1.00,1.06,1.14,1.24,1.36,1.50];
const RARITIES_COL=['#c7c7c7','#8fe08f','#8fc0ff','#d6a8ff','var(--ember-bright)','#ff96ae'];
...
const V03_SYNERGIES=[
 {id:'fellowship',name:'La Communauté',members:['aragorn','legolas','gimli'],desc:{fr:'+12% dégâts · +8% PV',en:'+12% damage · +8% HP',tr:'+12% hasar · +8% HP'},atk:0.12,hp:0.08},
 {id:'whiteCouncil',name:'Conseil Blanc',members:['gandalf','galadriel','elrond'],desc:{fr:'+15% magie · +10% mana',en:'+15% magic · +10% mana',tr:'+15% büyü · +10% mana'},magic:0.15,mana:0.10},
 {id:'gondor',name:'Héritiers du Gondor',members:['boromir','faramir','eowyn'],desc:{fr:'+10% critique · +10% défense',en:'+10% crit · +10% defense',tr:'+10% kritik · +10% savunma'},crit:0.10,mitig:0.10},
 {id:'rohan',name:'Cavaliers du Rohan',members:['theoden','eowyn'],desc:{fr:'+15% vitesse · +8% dégâts',en:'+15% speed · +8% damage',tr:'+15% hız · +8% hasar'},speed:0.15,atk:0.08},
 {id:'wild',name:'Gardiens Sauvages',members:['arwen','thranduil','beorn'],desc:{fr:'+12% soins · +8% PV',en:'+12% healing · +8% HP',tr:'+12% iyileştirme · +8% HP'},heal:0.12,hp:0.08}
];

const SAVE_KEY='lotr_jrpg_v1';
const SAVE_VERSION=5; // 2: zones/talents/races ; 3: langues/quêtes/invocation/synergies/prestige/hors-ligne ; 4: rareté des compagnons ; 5: tour/mines
let S=null;
...
function derivedOf(kind,id){
  ...
  // ==== SYNERGIES D'ÉQUIPE (héros + alliés) ====
  let syn=synergyBonus();
  atkP=Math.floor(atkP*(1+syn.atk)); atkM=Math.floor(atkM*(1+syn.atk+syn.magic));
  hp=Math.floor(hp*(1+syn.hp)); mana=Math.floor(mana*(1+syn.mana));
  crit=clamp(crit+syn.crit,0,0.65); mitig=clamp(mitig+syn.mitig,0,.75);
  spd=Math.floor(spd*(1+syn.speed)); healMul*=1+syn.heal;
...
function grantRewards(zone,boss,opt){
  ...
  // niveaux héros
  S.xp+=xp; let gained=0; while(S.xp>=xpNeeded(S.level)){ S.xp-=xpNeeded(S.level); S.level++; S.statPoints+=3; if(S.level%10===0)S.talentPoints=(S.talentPoints||0)+1; gained++; }
  // niveaux alliés
  let allyMsgs=[];
...
function renderDestinSection(){
  return `<div class="card"><div class="v03-grid">
    ...
    <button class="btn" onclick="openV03Synergies()">🤝 ${vtxt('synergy')}</button>
    <button class="btn" onclick="openV03Prestige()">🌑 ${vtxt('prestige')}</button>
  </div><div class="sub" style="text-align:center;margin-top:9px">${vtxt('daily')} · ${vtxt('summon')} · ${vtxt('synergy')} · ${vtxt('prestige')}</div></div>`;
}
...
function renderMap(){
  ...
  if(mapView==='v03synergies') return renderV03Synergies();
...
function renderTower(el){
  ...
  let html=`<div class="panel-title">🌑 Tour de Sauron</div>
    <div class="sub" style="text-align:center;margin-bottom:10px">Des étages sans fin, chacun plus dur que le précédent. Un gardien attend tous les 5 étages. Une défaite arrête l'ascension — seul le record compte.</div>
    <div class="card" style="text-align:center">
      <div style="font-size:36px">🗼</div>
      <div class="cz" style="font-size:20px;color:var(--ember-bright)">Étage ${floor}${guardianNext?' · Gardien':''}</div>
      <div class="sub">Record actuel : étage ${best}</div>
...
function towerWin(){
  ...
  let R=grantRewards({lvl:60+Math.floor(f/2),mobs:TOWER_MOBS,n:'Tour de Sauron'}, guardian, {lvBonus:Math.floor(f*0.4),mult:0.65+f*0.02});
  if(f>(S.towerBest||0))S.towerBest=f;
  TOWER.floor++; TOWER.inCombat=false;
  if(guardian)healTeamPct(0.35);
  save();
  showResult(true,Object.assign(R,{tower:true,floor:f,guardian,zoneName:'Tour de Sauron'}));
}
...
function renderPartyScreen(){
  ...
  if(knownUnsummoned.size===0&&mysteryZones.length===0)html+=`<div class="empty-note">Tu as rallié tous les héros connus.</div>`;
  [...knownUnsummoned].forEach(id=>{ let a=ALLY_BY_ID[id];
    html+=`<div class="inv-item"><span class="iico">${a.ico}</span>
...
function renderV03Quests(){
  ...
  html+=`<button class="btn ghost block" onclick="renderMap()">↩︎ ${v03t('Retour')}</button>`;el.innerHTML=html;
}
...
function activeSynergies(){let roster=new Set(S.party||['hero']);return V03_SYNERGIES.filter(s=>s.members.filter(x=>roster.has(x)).length>=s.members.length-1);}
function synergyBonus(){let out={atk:0,magic:0,hp:0,mana:0,crit:0,mitig:0,speed:0,heal:0};activeSynergies().forEach(s=>{for(let k in out)out[k]+=(s[k]||0);});return out;}
function renderV03Synergies(){let el=$('#screen-map');if(!el)return;let a=activeSynergies();let html=`<div class="panel-title">🤝 ${vtxt('synergy')}</div><div class="sub" style="text-align:center;margin-bottom:10px">${a.length?vtxt('synergyActive'):vtxt('noSynergy')}</div>`;V03_SYNERGIES.forEach(s=>{let active=a.includes(s),have=s.members.filter(x=>(S.party||[]).includes(x)).length;html+=`<div class="card" style="border-color:${active?'var(--ember)':'var(--line)'}"><div class="cz" style="color:${active?'var(--ember-bright)':'var(--bone)'}">${active?'✨ ':''}${s.name}</div><div class="sub">${s.members.map(id=>`${ALLY_BY_ID[id]?.ico||'👤'} ${ALLY_BY_ID[id]?.n||id}`).join(' · ')}</div><div class="chal-meta">${have}/${s.members.length} · ${s.desc[lang()]}</div></div>`;});html+=`<button class="btn ghost block" onclick="renderMap()">↩︎ ${v03t('Retour')}</button>`;el.innerHTML=html;}
window.openV03Synergies=()=>{mapView='v03synergies';renderV03Synergies();};

function summonPool(){
  let unlockedIds=new Set();
  ZONES.forEach(z=>{ if((S.reached||[]).includes(z.id)) (z.recruits||[]).forEach(id=>unlockedIds.add(id)); });
  return ALLIES.filter(a=>unlockedIds.has(a.id)&&!S.recruited.includes(a.id));
}
function renderV03Summon(){let el=$('#screen-map');if(!el)return;let pool=summonPool();let pity=Math.max(0,10-(S.summonPity||0));let oddsHtml=RARITIES.slice(0,6).map((r,i)=>`<span class="rar-${i}" style="color:${RARITIES_COL[i]}">${r.n}</span>`).join(' · ');let totalUnlockable=ZONES.reduce((n,z)=>n+(z.recruits||[]).length,0);let emptyMsg=(S.recruited.length>=totalUnlockable)?'Tu as recruté tous les héros connus.':'Explore de nouvelles régions pour révéler des héros à invoquer.';let html=`<div class="panel-title">✨ ${vtxt('summon')}</div><div class="card" style="text-align:center"><div style="font-size:36px">🌟</div><div class="cz" style="color:var(--ember-bright);font-size:16px">Les Appels de la Communauté</div><div class="sub" style="margin:7px 0">${pool.length?pool.slice(0,8).map(a=>`${a.ico} ${a.n}`).join(' · '):emptyMsg}</div><div class="sub" style="font-size:10px;margin-bottom:4px">${oddsHtml}</div><div class="sub">${vtxt('pity')} ${pity}</div><button class="btn gold block" style="margin-top:10px" onclick="summonV03()" ${S.ess<25||!pool.length?'disabled':''}>💎 ${vtxt('summonBtn')} · 25</button></div><button class="btn ghost block" onclick="renderMap()">↩︎ ${v03t('Retour')}</button>`;el.innerHTML=html;}
window.openV03Summon=()=>{mapView='v03summon';renderV03Summon();};
// rareté d'un compagnon invoqué : mêmes 6 paliers que le butin (RARITIES), pondérés
// pour rester généreux — invoquer coûte une ressource réelle, ça doit rester excitant.
function rollAllyRarity(guaranteed){
  let r=Math.random();
  let rar = r<0.03?5 : r<0.12?4 : r<0.30?3 : r<0.60?2 : r<0.85?1 : 0;
