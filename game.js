...
const V03_I18N={
  fr:{name:'Français',nav:{map:'Carte',party:'Équipe',bag:'Sac',tech:'Techniques',hero:'Héros'},
    strings:{'Terres du Milieu':'Terres du Milieu','Équipe':'Équipe','Sac':'Sac','Techniques':'Techniques','Héros':'Héros','Options':'Options','Musique':'Musique','Effets sonores':'Effets sonores','Activée':'Activée','Coupée':'Coupée','Activés':'Activés','Coupés':'Coupés','Retour':'Retour','Victoire':'Victoire','Défaite…':'Défaite…'}},
  en:{name:'English',nav:{map:'Map',party:'Party',bag:'Bag',tech:'Techniques',hero:'Hero'},
    strings:{'Terres du Milieu':'Middle-earth','Équipe':'Party','Sac':'Bag','Techniques':'Techniques','Héros':'Hero','Options':'Options','Musique':'Music','Effets sonores':'Sound effects','Activée':'On','Coupée':'Off','Activés':'On','Coupés':'Off','Retour':'Back','Victoire':'Victory','Défaite…':'Defeat…'}},
  tr:{name:'Türkçe',nav:{map:'Harita',party:'Ekip',bag:'Çanta',tech:'Yetenekler',hero:'Kahraman'},
    strings:{'Terres du Milieu':'Orta Dünya','Équipe':'Ekip','Sac':'Çanta','Techniques':'Yetenekler','Héros':'Kahraman','Options':'Ayarlar','Musique':'Müzik','Effets sonores':'Ses efektleri','Activée':'Açık','Coupée':'Kapalı','Activés':'Açık','Coupés':'Kapalı','Retour':'Geri','Victoire':'Zafer','Défaite…':'Yenilgi…'}}
};
const V03_TEXT={
 ...
 tr:{quests:'Görevler',summon:'Çağırma',synergy:'Sinerjiler',prestige:'Yükseliş',offline:'Çevrimdışı ilerleme',daily:'Günlük görevler',claim:'Al',summonBtn:'Çağır',summonCost:'Çağırma — 25 öz',pity:'Garantiye kalan',marks:'Gölge İşaretleri',prestigeBtn:'Yeniden Doğ',language:'Dil',dailyReset:'Her gün yenilenir',noQuest:'Görev yok',synergyActive:'Aktif sinerji',noSynergy:'Aktif sinerji yok',offlineCap:'maksimum 12 saat',offlineEmpty:'Çevrimdışı ilerleme yok.'}
};
const V03_QUESTS=[
...
function showResult(win,data){
  ...
  if(win){
    let lootHtml=(data.drops||[]).map(it=>`<div class="loot-pop rar-${it.rar}" style="color:inherit"><span style="color:${RARITIES_COL[it.rar]}">${ITEM_BASES[it.base].ico} ${it.name}</span></div>`).join('');
    ov.innerHTML=`<h2>${data.dungeon?'🏰 Donjon vaincu !':data.tower?(data.guardian?'🌑 Gardien vaincu !':'🌑 Étage franchi !'):'Victoire'}</h2>
      ${data.dungeon?`<div class="reward-line" style="color:var(--ember-bright)">${data.zoneName} — conquis de fond en comble</div>`:''}
      ${data.tower?`<div class="reward-line" style="color:var(--ember-bright)">Étage ${data.floor} de la Tour de Sauron</div>`:''}
      ${data.tower?`<div class="reward-line">🪙 <span class="v">${fmt(data.gold)}</span> or</div>`
        :`<div class="reward-line">🪙 <span class="v">${fmt(data.gold)}</span> or · ⚡ <span class="v">${fmt(data.xp)}</span> XP</div>`}
      <div class="reward-line">⛏️ <span class="v">${data.matG}</span> fragments${data.essG?` · 💎 <span class="v">${data.essG}</span> essence`:''}${data.tower?` · 🖤 <span class="v">${data.eclatsG}</span> Éclats`:''}</div>
...
function renderPartyScreen(){
  ...
  [...knownUnsummoned].forEach(id=>{ let a=ALLY_BY_ID[id];
    html+=a.banner
      ? `<div class="inv-item"><span class="iico">${a.ico}</span>
        <div class="imid"><div class="iname">${a.n} ${tchip(a.type)}</div><div class="iaff">${a.role} · exclusif à la 🎆 Bannière temporaire</div></div>
        <button class="btn sm banner-btn" onclick="openV03Banner()">🎆 Voir</button></div>`
      : `<div class="inv-item"><span class="iico">${a.ico}</span>
        <div class="imid"><div class="iname">${a.n} ${tchip(a.type)}</div><div class="iaff">${a.role} · révélé, prêt à invoquer</div></div>
        <button class="btn sm gold" onclick="openV03Summon()">✨ Invoquer</button></div>`; });
  mysteryZones.forEach(z=>{ let unknown=z.recruits.filter(r=>!S.recruited.includes(r));
    html+=`<div class="inv-item" style="opacity:.55"><span class="iico">❓</span>
      <div class="imid"><div class="iname">${unknown.length} héros inconnu(s)</div><div class="iaff">${z.n} — rejoins cette région pour les révéler</div></div></div>`; });
...
function renderV03Summon(){let el=$('#screen-map');if(!el)return;let pool=summonPool();let pity=Math.max(0,10-(S.summonPity||0));let oddsHtml=RARITIES.slice(0,5).map((r,i)=>`<span class="rar-${i}" style="color:${RARITIES_COL[i]}">${r.n}</span>`).join(' · ');let emptyMsg='Explore de nouvelles régions pour révéler des héros à invoquer.';let html=`<div class="panel-title">✨ ${vtxt('summon')}</div><div class="card" style="text-align:center"><div style="font-size:36px">🌟</div><div class="cz" style="color:var(--ember-bright);font-size:16px">Les Appels de la Communauté</div><div class="sub" style="margin:7px 0">${pool.length?pool.slice(0,8).map(a=>`${a.ico} ${a.n}`).join(' · '):emptyMsg}</div><div class="sub" style="font-size:10px;margin-bottom:4px">${oddsHtml}</div><div class="sub">${vtxt('pity')} ${pity}</div>
  ...
  <div class="sub" style="margin-top:8px;font-size:11px">Un compagnon déjà recruté ? Son <b>Ascension</b> progresse au lieu d'être perdu. Envie d'un compagnon de rareté Unique ? Rendez-vous sur la <b>🎆 Bannière</b>.</div></div><button class="btn ghost block" onclick="backMap()">↩︎ ${v03t('Retour')}</button>`;el.innerHTML=html;}
