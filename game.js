...
function load(){ try{ let r=localStorage.getItem(SAVE_KEY); if(!r)return false; let d=JSON.parse(r);
  ...
  // migration : races supprimées → race valide (5 races) ; réinitialise l'arbre
  if(!RACES[S.race]){
    ...
    S.race=map[S.race]||'humain';
    S.talents={}; // l'ancien arbre n'existe plus
    S.hpCur=null; S.manaCur=null;
  }
...
function v03EnsureState(){
  if(!S)return;
  S.v=3;
  if(!S.language)S.language='fr';
  if(!Array.isArray(S.shadowMarks))S.shadowMarks=[];
  if(typeof S.shadowMarks!=='number')S.shadowMarks=Number(S.shadowMarks)||0;
  if(!S.prestige)S.prestige={count:0,bonus:0};
  if(!S.prestige.count)S.prestige.count=0;
  if(!S.prestige.bonus)S.prestige.bonus=0;
  if(typeof S.summonPity!=='number')S.summonPity=0;
  if(!Array.isArray(S.questClaimed))S.questClaimed=[];
  if(S.questDate!==new Date().toISOString().slice(0,10)){S.questDate=new Date().toISOString().slice(0,10);S.questClaimed=[];}
...
const _v03Derived=derivedOf;
derivedOf=function(kind,id){let d=_v03Derived(kind,id),b=synergyBonus(),hero=kind==='hero'||kind==='ally';if(!hero)return d;d.atkP=Math.floor(d.atkP*(1+b.atk));d.atkM=Math.floor(d.atkM*(1+b.atk+b.magic));d.maxHp=Math.floor(d.maxHp*(1+b.hp));d.maxMana=Math.floor(d.maxMana*(1+b.mana));d.crit=clamp(d.crit+b.crit,0,0.65);d.mitig=clamp(d.mitig+b.mitig,0,.75);d.spd=Math.floor(d.spd*(1+b.speed));d.healMul*=1+b.heal;return d;};
...
function renderV03Summon(){let el=$('#screen-map');if(!el)return;let pool=summonPool();let pity=Math.max(0,10-(S.summonPity||0));let html=`<div class="panel-title">✨ ${vtxt('summon')}</div><div class="card" style="text-align:center"><div style="font-size:36px">🌟</div><div class="cz" style="color:var(--ember-bright);font-size:16px">Les Appels de la Communauté</div><div class="sub" style="margin:7px 0">${pool.length?pool.slice(0,8).map(a=>`${a.ico} ${a.n}`).join(' · '):'Tous les compagnons sont recrutés.'}</div><div class="sub">${vtxt('pity')} ${pity}</div><button class="btn gold block" style="margin-top:10px" onclick="summonV03()" ${S.ess<25||!pool.length?'disabled':''}>💎 ${vtxt('summonBtn')} · 25</button></div><button class="btn ghost block" onclick="renderMap()">↩︎ ${v03t('Retour')}</button>`;el.innerHTML=html;}
window.openV03Summon=()=>{mapView='v03summon';renderV03Summon();};
window.summonV03=function(){let pool=summonPool();if(S.ess<25||!pool.length)return;S.ess-=25;S.summonPity=(S.summonPity||0)+1;let guaranteed=S.summonPity>=10;let target=guaranteed?pool[0]:pick(pool);S.summonPity=guaranteed?0:S.summonPity;S.recruited.push(target.id);let ad=ensureAlly(target.id);ad.level=Math.max(1,S.level-1);if(S.party.length<3)S.party.push(target.id);S.dailySummon=(S.dailySummon||0)+1;sfx('level');flash('#e6a73e',.35);toast(`🌟 ${target.ico} ${target.n} rejoint la Communauté !`);renderV03Summon();renderTop();save();};
...
function v03ApplyMetaBonus(){let b=1+(S.prestige?.bonus||0);return b;}
const _v03GrantRewards=grantRewards;
grantRewards=function(zone,boss,opt){let r=_v03GrantRewards(zone,boss,opt);let b=v03ApplyMetaBonus();if(b>1){let extraG=Math.floor(r.gold*(b-1));let extraX=Math.floor(r.xp*(b-1));S.gold+=extraG;S.xp+=extraX;r.gold+=extraG;r.xp+=extraX;}if(S.dailyGold==null)S.dailyGold=0;S.dailyGold+=r.gold;if(S.dailyDrops==null)S.dailyDrops=0;S.dailyDrops+=r.drops.length;return r;};
const _v03Train=train;
window.train=function(){let before=S.level,c=_v03Train();if(S.gold>=0){S.dailyTrain=(S.dailyTrain||0)+1;}return c;};
...
function v03OpenMeta(which){if(which==='quests')openV03Quests();if(which==='summon')openV03Summon();if(which==='synergy')openV03Synergies();if(which==='prestige')openV03Prestige();}

/* --- extension de la carte : hub V0.3 --- */
const _v03RenderMap=renderMap;
renderMap=function(){if(mapView==='v03quests')return renderV03Quests();if(mapView==='v03summon')return renderV03Summon();if(mapView==='v03synergies')return renderV03Synergies();if(mapView==='v03prestige')return renderV03Prestige();_v03RenderMap();let el=$('#screen-map');if(!el)return;let card=ce('div','card');card.innerHTML=`<div class="cz" style="font-size:13px;color:var(--ember-bright);margin-bottom:8px">🌟 V0.3 — Destin</div><div class="v03-grid"><button class="btn gold" onclick="openV03Quests()">📜 ${vtxt('quests')}</button><button class="btn gold" onclick="openV03Summon()">✨ ${vtxt('summon')}</button><button class="btn" onclick="openV03Synergies()">🤝 ${vtxt('synergy')}</button><button class="btn" onclick="openV03Prestige()">🌑 ${vtxt('prestige')}</button></div><div class="sub" style="text-align:center;margin-top:7px">${vtxt('daily')} · ${vtxt('summon')} · ${vtxt('synergy')} · ${vtxt('prestige')}</div>`;el.appendChild(card);v03TranslateDOM();};

/* --- langue dans Héros --- */
const _v03Hero=renderHeroScreen;
renderHeroScreen=function(){_v03Hero();let el=$('#screen-hero');if(!el)return;let card=ce('div','card');card.innerHTML=`<div class="cz" style="font-size:13px;color:var(--ember-bright);margin-bottom:6px">🌍 ${vtxt('language')}</div><div class="v03-lang-row">${Object.entries(V03_I18N).map(([k,v])=>`<button class="btn sm ${lang()===k?'gold':''}" onclick="setV03Language('${k}')">${k==='fr'?'🇫🇷':k==='en'?'🇬🇧':'🇹🇷'} ${v.name}</button>`).join('')}</div>`;el.appendChild(card);v03TranslateDOM();};
...
function injectV03Styles(){if($('#v03Styles'))return;let st=document.createElement('style');st.id='v03Styles';st.textContent=`
.v03-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:7px}.v03-lang-row{display:flex;gap:7px;flex-wrap:wrap}.v03-lang-row .btn{flex:1;min-width:92px}.v03-modal{position:fixed;inset:0;background:rgba(4,2,10,.82);z-index:100;display:flex;align-items:center;justify-content:center;padding:18px;backdrop-filter:blur(5px)}.v03-modal-box{width:min(430px,100%);background:linear-gradient(180deg,#2c2348,#120d20);border:1px solid var(--ember);border-radius:18px;padding:20px;box-shadow:0 20px 80px #000}.offline-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin:15px 0}.offline-grid>div{padding:13px;text-align:center;background:rgba(0,0,0,.28);border:1px solid var(--line);border-radius:12px;font-size:22px}.offline-grid b{display:block;color:var(--ember-bright);font-family:'Cinzel';font-size:15px;margin-top:3px}.offline-grid small{display:block;color:var(--muted);font-size:10px}.v03-grid .btn{min-height:46px}
`;document.head.appendChild(st);}
function initV03(){if(!S||!S.created)return;v03EnsureState();injectV03Styles();let r=processOfflineV03();if(r)setTimeout(()=>showOfflineV03(r),250);renderTop();if(!$('#screen-map').classList.contains('hidden'))renderMap();else renderHeroScreen();}
setTimeout(initV03,0);
