...
function renderMapTabs(){
  ...
  return `<div class="map-tabs">${tabs.map(([id,ico,label])=>
    `<button class="map-tab ${mapSection===id?'on':''}" onclick="setMapSection('${id}')">${ico} ${label}${mapTabBadge(id)?' <span class="badge-dot">●</span>':''}</button>`
  ).join('')}</div>`;
}
  ...
  ZONES.forEach((z,i)=>{
    ...
    html+=`<div class="zone-node ${unlocked?'':'locked'} ${cur?'current':''}" ${unlocked?`onclick="goZone('${z.id}')"`:''}>
      ...
      <div class="zmid">
        ...
        <div class="zmeta">Niveau conseillé ${z.lvl}${(unlocked&&!reached)?' · 🥾 sentier à parcourir':''}${(z.recruits&&z.recruits.length&&unlocked)?` · Débloque pour l'invocation : ${z.recruits.map(r=>ALLY_BY_ID[r].n).join(', ')}`:''}</div>
      </div>
...
function renderDestinSection(){
  ...
  <div class="sub" style="text-align:center;margin-top:9px">${vtxt('daily')} · ${vtxt('summon')} · Bannière · ${vtxt('prestige')}</div></div>`;
}
function renderModesSection(){
  ...
  return `<div class="card mode-card" onclick="openTower()">
      ...
      <div class="mode-card-mid"><div class="mode-card-n">Tour de Sauron</div><div class="mode-card-d">Étages sans fin, toujours plus durs. Record : étage ${best}.</div></div>
      <div class="mode-card-go">▸</div>
    </div>
    <div class="card mode-card" onclick="openMines()">
      <div class="mode-card-ico">⛏️</div>
      <div class="mode-card-mid"><div class="mode-card-n">Mines</div><div class="mode-card-d">Production passive de ressources.${minesAmt>0?` <b style="color:var(--ember-bright)">${fmt(minesAmt)} prêt${minesAmt>1?'s':''} à récolter</b>`:''}</div></div>
      <div class="mode-card-go">▸</div>
    </div>
    <div class="sub" style="text-align:center;margin-top:4px">D'autres modes (Raid, Arène) arriveront bientôt.</div>`;
}
...
function showZoneLore(zoneId){
  ...
  bg.innerHTML=`<div class="modal lore-modal">
      <h3>${zone.ico} ${zone.n}</h3>
      <div class="lore-text">${zone.lore||zone.desc}</div>
      <button class="btn gold block" style="margin-top:14px" onclick="this.closest('.modal-bg').remove();confirmZoneLore('${zoneId}')">Entrer ▸</button>
    </div>`;
...
function renderTower(el){
  ...
  let html=`<div class="panel-title">🌑 Tour de Sauron</div>
    <div class="sub" style="text-align:center;margin-bottom:10px">Des étages sans fin, chacun plus dur que le précédent. Un gardien attend tous les 5 étages. Une défaite arrête l'ascension — seul le record compte.
    <br><b style="color:var(--bone)">Aucune expérience gagnée ici</b> — la Tour rapporte de l'équipement et des 🖤 <b>Éclats des Anneaux</b>, la ressource qui permet de forger un Porteur de l'Anneau.</div>
    <div class="card" style="text-align:center">
      <div style="font-size:36px">🗼</div>
      <div class="cz" style="font-size:20px;color:var(--ember-bright)">Étage ${floor}${guardianNext?' · Gardien':''}</div>
      <div class="sub">Record actuel : étage ${best} · 🖤 ${fmt(S.eclats||0)} Éclats</div>
      <button class="btn gold block" style="margin-top:10px" onclick="${TOWER?'towerFight()':'towerStart()'}">${TOWER?"⚔️ Continuer l'ascension":'🌑 Commencer l\'ascension'}</button>
      ${TOWER?`<button class="btn ghost block" style="margin-top:6px" onclick="towerAbandon()">Abandonner (le record reste acquis)</button>`:''}
    </div>
...
function renderMines(el){
  ...
  let html=`<div class="panel-title">⛏️ Mines</div>
    <div class="sub" style="text-align:center;margin-bottom:10px">Aucun combat ici : les mines produisent des fragments en continu, même quand tu es ailleurs dans le jeu (plafond 12h de production).</div>
    <div class="card" style="text-align:center">
      <div style="font-size:36px">⛏️</div>
      <div class="cz" style="font-size:13px">Palier ${lvl} · ${minesRate()} fragments/h</div>
      <div style="font-size:26px;color:var(--ember-bright);font-family:'Cinzel';margin:8px 0">${fmt(pending)} en attente</div>
      <button class="btn gold block" onclick="minesCollect()" ${pending<1?'disabled':''}>⛏️ Récolter</button>
      ${upCost?`<button class="btn block" style="margin-top:8px" onclick="minesUpgrade()" ${S.gold<upCost?'disabled':''}>⬆ Améliorer — 🪙 ${fmt(upCost)} (palier ${lvl+1})</button>`:`<div class="sub" style="margin-top:8px">Palier maximum atteint.</div>`}
    </div>
    <button class="btn ghost block" onclick="mapView='map';renderMap()">↩︎ ${v03t('Retour')}</button>`;
  el.innerHTML=html;
}
window.minesCollect=function(){ let p=minesPending(); if(p<1)return; S.mat+=p; S.minesLastCollect=Date.now(); sfx('level'); toast(`⛏️ +${fmt(p)} fragments`); renderMines($('#screen-map')); renderTop(); save(); };
window.minesUpgrade=function(){ let lvl=S.minesLevel||1; let cost=MINES_UPGRADE_COST[lvl]; if(!cost||S.gold<cost)return; S.mat+=minesPending(); S.minesLastCollect=Date.now(); S.gold-=cost; S.minesLevel=lvl+1; sfx('buff'); toast(`⛏️ Mines — palier ${lvl+1} !`); renderMines($('#screen-map')); renderTop(); save(); };
...
function renderPartyScreen(){
  ...
  html+=`<div class="divider">En première ligne (${S.party.length}/${PARTY_MAX})</div>`;
...
function memberRow(ref,active){
  ...
  return `<div class="inv-item rar-${rar}"><span class="iico">${a.ico}</span>
    <div class="imid"><div class="iname">${a.n} <span class="rar-${rar}" style="color:inherit;font-size:10px;opacity:.9">${RARITIES[rar].n}</span> ${tchip(a.type)}</div>
    ...
    <div style="display:flex;flex-direction:column;gap:4px">
      ...
      ${active?`<button class="btn sm ghost" onclick="benchAlly('${ref}')">Retirer</button>`
              :`<button class="btn sm ghost" onclick="activateAlly('${ref}')" ${S.party.length>=PARTY_MAX?'disabled':''}>Aligner</button>`}
    </div></div>`;
}
...
function renderCharDetail(el,id){
  ...
  if(!isHero){
    ...
    html+=`<div class="card" style="text-align:center">
      <div class="pnum" style="font-size:11px;color:var(--muted)">⬆️ Ascension (doublons d'invocation)</div>
      <div style="font-size:18px;margin-top:2px">${pips}</div>
      <div class="sub" style="font-size:10px;margin-top:2px">${asc>=5?'Maximum atteint':`+${asc*6}% de stats · encore un doublon pour progresser`}</div>
    </div>`;
    let sp=ALLY_SPECIALS[id];
    if(sp){
      let unlocked=rar>=3;
      html+=`<div class="ally-special ${unlocked?'':'locked'}">
        <div class="asn">${unlocked?'✨':'🔒'} ${sp.n}</div>
        <div class="asd">${unlocked?sp.desc:`Débloqué à partir de la rareté Épique (actuellement ${RARITIES[rar].n}).`}</div>
      </div>`;
    }
  }
  // stats allocation
  ...
  if(!isHero){
    ...
    html+=`<div style="display:flex;gap:8px">
      ${active?`<button class="btn ghost block" onclick="benchAlly('${id}');closeCharDetail()">Mettre en réserve</button>`
              :`<button class="btn gold block" onclick="activateAlly('${id}');renderPartyScreen()" ${S.party.length>=PARTY_MAX?'disabled':''}>Aligner au combat</button>`}</div>`;
...
window.activateAlly=function(ref){ if(S.party.length>=PARTY_MAX){toast(`Équipe pleine (${PARTY_MAX} max).`);return;} if(!S.party.includes(ref))S.party.push(ref); sfx('click'); renderPartyScreen(); save(); };
...
function renderBag(){
  ...
  html+=`<div class="divider">Inventaire ${bagFilter?'· '+SLOTS.find(x=>x.id===bagFilter).n:''} (${list.length})</div>`;
  if(list.length===0)html+=`<div class="empty-note">${bagFilter?'Aucun objet pour cet emplacement.':'Ton sac est vide. Les ennemis et boss laissent tomber de l\'équipement.'}</div>`;
  list.forEach(it=>{ let b=ITEM_BASES[it.base];
    html+=`<div class="inv-item rar-${it.rar}">
      ...
      <div style="display:flex;flex-direction:column;gap:4px">
        ...
        ${it.unique?'':`<button class="btn sm ghost" onclick="rerollItem(${it.iid})">🎲 ${rerollCost(it)} frag</button>`}
...
window.rerollItem=function(iid){ let it=S.inv.find(x=>x.iid===iid); if(!it||it.unique)return; let cost=rerollCost(it);
  ...
  sfx('magic'); toast(`🎲 ${it.name} — caractéristiques relancées !`); renderBag(); renderTop(); save(); };
...
function renderHeroScreen(){
  ...
  if(ev.tier===0){
    ...
    html+=`<div class="card" style="border:1px solid var(--line2)">
      <div class="cz" style="font-size:13px;color:var(--ember-bright);margin-bottom:4px">${ev.tier>=3?'🌟 Évolution ultime':'⭐ Évolution'} — palier ${ev.tier}</div>
      <div class="sub" style="font-size:11px;margin-bottom:8px">Requis : niveau ${ev.req} · 💎 ${ev.cost} essence ${ok?'':'<span style="color:var(--blood)">(non rempli)</span>'}. +${EVO_BOOST[ev.tier]} à toutes les stats${ev.tier===1?', un arbre élémentaire et le choix de ta voie.':' (poursuite de ta branche).'}</div>`;
    ev.opts.forEach(opt=>{
      html+=`<div class="evo-opt">
        <div class="evo-mid"><div class="evo-n">${opt.ico} ${opt.n}</div>${opt.passive?`<div class="evo-d">${opt.passive}</div>`:''}</div>
...
function renderV03Banner(){
  ...
  let html=`<div class="panel-title">🎆 Bannières Temporaires</div>
    <div class="sub" style="text-align:center;margin-bottom:8px">Chaque bannière est disponible en même temps que les autres, avec sa propre garantie. C'est la seule voie vers un compagnon de rareté <span style="color:${RARITIES_COL[5]}">Unique</span>.</div>`;
  if(!roster.length){
    html+=`<div class="card" style="text-align:center"><div class="sub">Aucun héros de bannière découvert pour l'instant. Explore de nouvelles régions.</div></div>`;
  } else {
    roster.forEach(featured=>{
      ...
      html+=`<div class="card banner-card" style="text-align:center">
        ...
        <div class="summon-name rar-5" style="margin-top:2px">${featured.n}</div>
        <div class="sub">${featured.role||''} ${tchip(featured.type)}</div>
        ${already?`<div class="reward-line" style="margin-top:8px">✓ Dans la Communauté — Ascension ${ad.ascend||0}/5. Les invocations suivantes la font progresser.</div>`:
          `<div class="sub" style="margin:8px 0">${ALLY_SPECIALS[featured.id]?`Talent exclusif : <b>${ALLY_SPECIALS[featured.id].n}</b> — ${ALLY_SPECIALS[featured.id].desc}`:''}</div>`}
        <div class="sub">Garantie dans ${pityLeft} invocation${pityLeft>1?'s':''}</div>
        <button class="btn gold block" style="margin-top:10px" onclick="summonBanner('${featured.id}')" ${S.ess<40?'disabled':''}>💎 Invoquer · 40</button>
    ...
    html+=`<div class="sub" style="text-align:center;margin-top:2px">D'autres héros de bannière rejoindront la liste au fil des mises à jour.</div>`;
  }
...
function renderRingForge(){
  ...
  let html=`<div class="panel-title">👑 La Forge de l'Anneau</div>
    <div class="sub" style="text-align:center;margin-bottom:10px">Une rareté à part — <b style="color:#ffcf5c">Anneau</b> — réservée aux Porteurs de l'Anneau. Douze existeront un jour ; ils rejoindront la Forge un à un. On ne les invoque pas : on les <i>forge</i>, avec les 🖤 Éclats des Anneaux rapportés de la Tour de Sauron.</div>`;
  if(!target){
    let owned=RING_CHARACTERS.filter(id=>S.recruited.includes(id));
    html+=`<div class="card ring-card"><div style="font-size:40px">👑</div><div class="cz" style="color:#ffcf5c">Tous les Porteurs connus sont forgés</div><div class="sub" style="margin-top:6px">${owned.length}/${RING_CHARACTERS.length} Porteur(s) — d'autres arriveront lors de prochaines mises à jour.</div></div>`;
  } else {
    ...
    html+=`<div class="card ring-card">
      ...
      <div class="summon-name rar-6" style="margin-top:2px">${target.n}</div>
      <div class="sub">${target.role||''}</div>
      ${ALLY_SPECIALS[target.id]?`<div class="ally-special" style="margin-top:10px;text-align:left"><div class="asn">✨ ${ALLY_SPECIALS[target.id].n}</div><div class="asd">${ALLY_SPECIALS[target.id].desc}</div></div>`:''}
      <div class="ring-progress"><i style="width:${pct}%"></i></div>
      <div class="sub">🖤 ${fmt(cur)} / ${fmt(cost)} Éclats des Anneaux</div>
      <button class="btn ring-btn block" style="margin-top:10px" onclick="forgeRing()" ${ready?'':'disabled'}>${ready?"👑 Forger l'Anneau":'Éclats insuffisants'}</button>
    </div>
    <div class="sub" style="text-align:center;margin-top:6px">Récolte tes Éclats en gravissant la 🌑 Tour de Sauron.</div>`;
  }
  html+=`<button class="btn ghost block" style="margin-top:10px" onclick="backMap()">↩︎ ${v03t('Retour')}</button>`;
  el.innerHTML=html;
}
...
window.forgeRing=function(){
  ...
  if(it)setTimeout(()=>toast(`💍 ${it.name} rejoint ton sac !`),700);
};

function canPrestige(){return S.level>=60&&S.cleared.includes('z8');}
function renderV03Prestige(){let el=$('#screen-map');if(!el)return;let p=S.prestige||{count:0,bonus:0};let html=`<div class="panel-title">🌑 ${vtxt('prestige')}</div><div class="card"><div class="cz">${vtxt('marks')} : <b style="color:var(--ember-bright)">${fmt(S.shadowMarks)}</b></div><div class="sub" style="margin-top:7px">Renaître remet le niveau, l’or et la progression des zones à zéro, mais conserve les personnages, l’équipement, les talents et tes Marques de l’Ombre.</div><div class="reward-line" style="margin-top:10px">Bonus permanent actuel : <b>+${Math.round(p.bonus*100)}%</b> dégâts & récompenses</div><button class="btn gold block" style="margin-top:12px" onclick="prestigeV03()" ${canPrestige()?'':'disabled'}>${vtxt('prestigeBtn')} · +10 Marques</button><div class="sub" style="margin-top:7px">Condition : niveau 60 + Barad-dûr conquis.</div></div><button class="btn ghost block" onclick="backMap()">↩︎ ${v03t('Retour')}</button>`;el.innerHTML=html;}
