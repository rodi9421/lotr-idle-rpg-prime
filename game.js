...
const ZONES=[
  ...
  {id:'z9',n:'Ruines d\'Angmar',ico:'🏚️',type:'tenebres',lvl:72,mobs:['spectre','liche_mineure','nazgul_mob','demon'],boss:'roi_spectre',
   loot:['lame_morgul','couronne_roi','capuche_ombre'],recruits:[],
...
const SAVE_VERSION=6; // 2: zones/talents/races ; 3: langues/quêtes/invocation/prestige/hors-ligne ; 4: rareté des compagnons ; 5: tour/mines ; 6: bannière, Anneau, Éclats, lore, retrait synergies
let S=null;
...
function defaultState(){
  return {
    ...
    eclats:0, bannerPity:0, loreSeen:[],
  };
}
...
function derivedOf(kind,id){
  ...
  if(kind==='ally'){ let rar=ensureAlly(id).rar; if(typeof rar!=='number')rar=2;
...
function renderDestinSection(){
  return `<div class="card"><div class="v03-grid">
    ...
    <button class="btn banner-btn" onclick="openV03Banner()">🎆 Bannière${bannerPityLeft()<=2?' <span class="badge-dot">●</span>':''}</button>
...
function renderCharDetail(el,id){
  ...
  if(!isHero){
    let rar= typeof ensureAlly(id).rar==='number'?ensureAlly(id).rar:2;
    let sp=ALLY_SPECIALS[id];
    if(sp){
      let unlocked=rar>=3;
...
// Le pool d'invocation standard exclut les personnages « bannière » (rareté Unique exclusive,
// voir bannerPool()) — ils ne s'obtiennent que via la Bannière temporaire, comme sur Genshin.
function summonPool(){
  let unlockedIds=new Set();
  ZONES.forEach(z=>{ if((S.reached||[]).includes(z.id)) (z.recruits||[]).forEach(id=>unlockedIds.add(id)); });
  return ALLIES.filter(a=>unlockedIds.has(a.id)&&!S.recruited.includes(a.id)&&!a.banner&&!a.ring);
}
function renderV03Summon(){let el=$('#screen-map');if(!el)return;let pool=summonPool();let pity=Math.max(0,10-(S.summonPity||0));let oddsHtml=RARITIES.slice(0,5).map((r,i)=>`<span class="rar-${i}" style="color:${RARITIES_COL[i]}">${r.n}</span>`).join(' · ');let totalUnlockable=ZONES.reduce((n,z)=>n+(z.recruits||[]).length,0);let emptyMsg=(S.recruited.length>=totalUnlockable)?'Tu as recruté tous les héros connus.':'Explore de nouvelles régions pour révéler des héros à invoquer.';let html=`<div class="panel-title">✨ ${vtxt('summon')}</div><div class="card" style="text-align:center"><div style="font-size:36px">🌟</div><div class="cz" style="color:var(--ember-bright);font-size:16px">Les Appels de la Communauté</div><div class="sub" style="margin:7px 0">${pool.length?pool.slice(0,8).map(a=>`${a.ico} ${a.n}`).join(' · '):emptyMsg}</div><div class="sub" style="font-size:10px;margin-bottom:4px">${oddsHtml}</div><div class="sub">${vtxt('pity')} ${pity}</div><button class="btn gold block" style="margin-top:10px" onclick="summonV03()" ${S.ess<25||!pool.length?'disabled':''}>💎 ${vtxt('summonBtn')} · 25</button><div class="sub" style="margin-top:8px;font-size:11px">Envie d'un compagnon de rareté Unique ? Rendez-vous sur la <b>🎆 Bannière</b>.</div></div><button class="btn ghost block" onclick="backMap()">↩︎ ${v03t('Retour')}</button>`;el.innerHTML=html;}
...
window.summonV03=function(){let pool=summonPool();if(S.ess<25||!pool.length)return;S.ess-=25;S.summonPity=(S.summonPity||0)+1;let guaranteed=S.summonPity>=10;let target=guaranteed?pool[0]:pick(pool);S.summonPity=guaranteed?0:S.summonPity;S.recruited.push(target.id);let ad=ensureAlly(target.id);ad.level=Math.max(1,S.level-1);let rar=rollAllyRarity(guaranteed);ad.rar=rar;if(S.party.length<3)S.party.push(target.id);S.dailySummon=(S.dailySummon||0)+1;renderV03Summon();renderTop();save();playSummonReveal(target,rar,guaranteed);};

/* ===================================================================
   BANNIÈRE TEMPORAIRE (façon Genshin) — seule source des compagnons
   de rareté Unique. Un compagnon « bannière » (Gandalf, Galadriel…)
   est mis en avant, tourne automatiquement tous les 14 jours, et ne
   peut JAMAIS être obtenu via l'invocation standard.
   =================================================================== */
const BANNER_ROTATION_DAYS=14;
function bannerRoster(){ return ALLIES.filter(a=>a.banner); }
function currentBannerAlly(){
  let roster=bannerRoster(); if(!roster.length)return null;
  let idx=Math.floor(Date.now()/(BANNER_ROTATION_DAYS*86400000))%roster.length;
  return roster[idx];
}
function bannerTimeLeft(){
  let span=BANNER_ROTATION_DAYS*86400000;
  let left=span-(Date.now()%span);
  return Math.ceil(left/86400000);
}
function bannerPityLeft(){ return Math.max(0,10-(S.bannerPity||0)); }
window.openV03Banner=()=>{mapView='v03banner';renderV03Banner();};
function renderV03Banner(){
  let el=$('#screen-map'); if(!el)return;
  let featured=currentBannerAlly();
  let pityLeft=bannerPityLeft();
  let already=featured&&S.recruited.includes(featured.id);
  let html=`<div class="panel-title">🎆 Bannière Temporaire</div>`;
  if(!featured){
    ...
    html+=`<div class="card banner-card" style="text-align:center">
      <div class="banner-timer">⏳ Nouvelle bannière dans ${bannerTimeLeft()} j</div>
      ...
      ${already?`<div class="reward-line" style="margin-top:8px">✓ Déjà rejoint la Communauté — les invocations suivantes rapportent de l'essence de consolation.</div>`:
        `<div class="sub" style="margin:8px 0">Seule cette bannière permet d'obtenir un compagnon de rareté <span class="rar-5" style="color:${RARITIES_COL[5]}">Unique</span>.
        ${ALLY_SPECIALS[featured.id]?`Talent exclusif : <b>${ALLY_SPECIALS[featured.id].n}</b> — ${ALLY_SPECIALS[featured.id].desc}`:''}</div>`}
      <div class="sub">Garantie dans ${pityLeft} invocation${pityLeft>1?'s':''}</div>
      <button class="btn gold block" style="margin-top:10px" onclick="summonBanner()" ${S.ess<40?'disabled':''}>💎 Invoquer sur la Bannière · 40</button>
    </div>
    <div class="sub" style="text-align:center;margin-top:6px">D'autres héros de bannière rejoindront la rotation au fil des mises à jour.</div>`;
  }
  html+=`<button class="btn ghost block" style="margin-top:10px" onclick="backMap()">↩︎ ${v03t('Retour')}</button>`;
  el.innerHTML=html;
}
window.summonBanner=function(){
  let featured=currentBannerAlly(); if(!featured||S.ess<40)return;
  S.ess-=40; S.bannerPity=(S.bannerPity||0)+1;
  let guaranteed=S.bannerPity>=10;
  let alreadyHave=S.recruited.includes(featured.id);
  // 50% de chance d'obtenir le héros de bannière (100% si garanti ou déjà possédé) ; sinon,
  // essence de consolation — jamais un pull « pour rien ».
  let win=guaranteed||alreadyHave||Math.random()<0.5;
  if(win){
    S.bannerPity=0;
    if(!alreadyHave){
      S.recruited.push(featured.id);
      let ad=ensureAlly(featured.id); ad.level=Math.max(1,S.level-1); ad.rar=5;
      if(S.party.length<3)S.party.push(featured.id);
      S.dailySummon=(S.dailySummon||0)+1;
      renderV03Banner(); renderTop(); save();
      playSummonReveal(featured,5,guaranteed);
      return;
    }
  }
  // déjà possédé, ou 50/50 perdu : consolation en essence pour ne jamais laisser un pull vide.
  let consolation=alreadyHave?15:8;
  S.ess+=consolation; renderV03Banner(); renderTop(); save();
  toast(`💎 +${consolation} essence de consolation.`);
};
...
function playSummonReveal(target,rar,guaranteed){
  ...
  btn.onclick=()=>{ ov.classList.remove('on'); setTimeout(()=>ov.remove(),260);
    toast(`${target.ico} ${target.n} (${RARITIES[rar].n}) rejoint la Communauté !`); };
}
...
window.forgeRing=function(){
  ...
  if(S.party.length<3)S.party.push(target.id);
