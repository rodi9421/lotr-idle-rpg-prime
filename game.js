...
const SAVE_VERSION=4; // 2: zones/talents/races ; 3: langues/quêtes/invocation/synergies/prestige/hors-ligne ; 4: rareté des compagnons
let S=null;
...
function showResult(win,data){
  ...
  if(win){
    ...
    ov.innerHTML=`<h2>Défaite…</h2>
      <div class="reward-line">${data.inPath?'Le sentier t\'a vaincu. Tu perds ta progression et ':'Tu perds '}${fmt(data.lost)} or et bats en retraite.</div>
      <div style="display:flex;gap:8px;margin-top:18px">
...
function renderMap(){
  ...
  // hub de méta-progression : quêtes, invocation, synergies, ascension
  html+=`<div class="card"><div class="cz" style="font-size:13px;color:var(--ember-bright);margin-bottom:8px">🌟 Destin</div><div class="v03-grid"><button class="btn gold" onclick="openV03Quests()">📜 ${vtxt('quests')}</button><button class="btn gold" onclick="openV03Summon()">✨ ${vtxt('summon')}</button><button class="btn" onclick="openV03Synergies()">🤝 ${vtxt('synergy')}</button><button class="btn" onclick="openV03Prestige()">🌑 ${vtxt('prestige')}</button></div><div class="sub" style="text-align:center;margin-top:7px">${vtxt('daily')} · ${vtxt('summon')} · ${vtxt('synergy')} · ${vtxt('prestige')}</div></div>`;
  el.innerHTML=html;
  v03TranslateDOM();
}
...
function summonPool(){return ALLIES.filter(a=>!S.recruited.includes(a.id));}
function renderV03Summon(){let el=$('#screen-map');if(!el)return;let pool=summonPool();let pity=Math.max(0,10-(S.summonPity||0));let oddsHtml=RARITIES.slice(0,6).map((r,i)=>`<span class="rar-${i}" style="color:${RARITIES_COL[i]}">${r.n}</span>`).join(' · ');let html=`<div class="panel-title">✨ ${vtxt('summon')}</div><div class="card" style="text-align:center"><div style="font-size:36px">🌟</div><div class="cz" style="color:var(--ember-bright);font-size:16px">Les Appels de la Communauté</div><div class="sub" style="margin:7px 0">${pool.length?pool.slice(0,8).map(a=>`${a.ico} ${a.n}`).join(' · '):'Tous les compagnons sont recrutés.'}</div><div class="sub" style="font-size:10px;margin-bottom:4px">${oddsHtml}</div><div class="sub">${vtxt('pity')} ${pity}</div><button class="btn gold block" style="margin-top:10px" onclick="summonV03()" ${S.ess<25||!pool.length?'disabled':''}>💎 ${vtxt('summonBtn')} · 25</button></div><button class="btn ghost block" onclick="renderMap()">↩︎ ${v03t('Retour')}</button>`;el.innerHTML=html;}
