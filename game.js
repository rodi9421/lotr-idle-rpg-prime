...
const TECH=[
  ...
  // ---- TENEBRES (accessibles via race, Anneau ou Nazgûl) ----
  {id:'te1',n:'Lame d\'Ombre',ico:'🌑',type:'tenebres',kind:'attack',pow:1.45,cost:8,stat:'esprit',target:'one',lvl:1,status:'vuln',vfx:'shadowbolt',desc:'Rend vulnérable.'},
  {id:'te2',n:'Drain de Vie',ico:'🩸',type:'tenebres',kind:'attack',pow:1.5,cost:16,stat:'esprit',target:'one',lvl:5,drain:0.5,vfx:'drain',desc:'Vole la vie.'},
  {id:'te3',n:'Néant',ico:'⚫',type:'tenebres',kind:'attack',pow:1.25,cost:20,stat:'esprit',target:'all',lvl:10,status:'vuln',vfx:'voidnova',desc:'Engloutit tous.'},
  {id:'te4',n:'Apocalypse',ico:'💀',type:'tenebres',kind:'attack',pow:3.6,cost:34,stat:'esprit',target:'all',lvl:18,vfx:'voidnova',nazgul:true,desc:'Anéantissement (Nazgûl).'},
  // ---- DOUBLE-TYPE ----
...
const ZONES=[
  {id:'z1',n:'Terres Sauvages',ico:'🌄',type:'nature',lvl:1,mobs:['gobelin','loup'],boss:'warg',
   loot:['epee_rouillee','tunique_cuir','botte_usee'],recruits:['aragorn'],
  ...
  {id:'z2',n:'Forêt de Vertbois',ico:'🌲',type:'nature',lvl:5,mobs:['araignee','esprit','loup'],boss:'sorciere',
   loot:['arc_elfe','cape_sylvestre','amulette_feuille'],recruits:['legolas'],
  ...
  {id:'z3',n:'Mines de la Moria',ico:'⛏️',type:'feu',lvl:10,mobs:['orque','troll','balrog_spawn'],boss:'balrog',
   loot:['hache_naine','plastron_mithril','anneau_braise'],recruits:['gimli','gandalf'],
  ...
  {id:'z4',n:'Lothlórien',ico:'🌟',type:'lumiere',lvl:16,mobs:['esprit','spectre','araignee'],boss:'saroumane',
   loot:['baton_blanc','robe_etoilee','amulette_galadhrim'],recruits:['galadriel','elrond'],
  ...
  {id:'z5',n:'Champs du Pelennor',ico:'⚔️',type:'physique',lvl:24,mobs:['uruk','orque','nazgul_mob','golem'],boss:'roi_spectre',
   loot:['lame_runique','heaume_gondor','anneau_vaillance'],recruits:['eowyn'],
  ...
  {id:'z6',n:'Mordor — le Mont Destin',ico:'🌋',type:'tenebres',lvl:34,mobs:['nazgul_mob','spectre','balrog_spawn','demon'],boss:'sauron',
   loot:['lame_morgul','armure_obsidienne','anneau_unique'],recruits:['arwen'],
...
function derivedOf(kind,id){
  ...
  if(race){ let p=race.pid;
    ...
    if(p==='crit')crit+=0.08; if(p==='heal')healMul=1.2;
  }
  if(kind==='hero'&&S.isNazgul){ atkM*=1.25; mana*=1.15; }
...
function buildEnemies(zone,boss){
  ...
  for(let i=0;i<count;i++){ let mk=pick(zone.mobs); let lv=zone.lvl+ri(0,3);
    let e=mkMob(mk,lv,1); e.ref='e'+i; arr.push(e); }
  return arr;
}
...
function dungeonStep(){
  ...
  // difficulté qui grimpe au fil des étapes (sans sur-niveler les zones basses)
  let diff=1+ (s-1)*0.05;             // +5% par étape
...
function checkEnd(){
  ...
  if(aliveEnemies().length===0){
    ...
    if(RUN) dungeonVictory(); else victory();
    return true;
  }
...
function doMove(actor,t,target){
  ...
  if(t.kind==='heal'){
    ...
    // corruption si le héros use les ténèbres (lentement)
    if(actor.isHero&&!S.isNazgul&&techTypes(t).includes('tenebres')){ addCorruption(1.5); }
  }
...
function defeat(){
  ...
  save();
  showResult(false,{lost,inDungeon});
}
...
function showResult(win,data){
  ...
  if(win){
    ...
    sfx('win');
    if(auto&&!data.dungeon){ setTimeout(()=>{ if($('#resultOverlay').classList.contains('on'))afterBattle('again'); },1500); }
  } else {
...
function renderMap(){
  ...
  ZONES.forEach((z,i)=>{
    ...
    html+=`<div class="zone-node ${unlocked?'':'locked'} ${cur?'current':''}" ${unlocked?`onclick="goZone('${z.id}')"`:''}>
      ...
      <div class="zmid">
        ...
        <div class="zmeta">Niveau conseillé ${z.lvl}${(z.recruits&&z.recruits.length&&unlocked)?` · Recrues : ${z.recruits.map(r=>ALLY_BY_ID[r].n).join(', ')}`:''}</div>
      </div>
  ...
  html+=`<div class="card" style="position:sticky;bottom:0">
    ...
    <div style="text-align:center;margin-top:8px;font-size:12px;color:var(--muted)">PV ${fmt(Math.max(0,S.hpCur||0))}/${fmt(heroDerived().maxHp)} · Mana ${fmt(S.manaCur||0)}/${fmt(heroDerived().maxMana)}
      <button class="btn sm ghost" style="margin-left:8px" onclick="rest()">🏕️ Repos équipe (10 or)</button></div>
  </div>`;
  el.innerHTML=html;
}
...
window.fightBoss=function(){ ac(); Music.ensure(); startDungeon(ZONE_BY_ID[S.curZone]); };
...
function canLearnTech(t){
  ...
  // double-type : il faut maîtriser au moins un des deux arbres
  return techTypes(t).some(ty=>avail.includes(ty));
}
function renderTech(){
