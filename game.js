...
/* ===================================================================
   ARBRES DE TALENTS — points gagnés à chaque niveau, bonus passifs
   =================================================================== */
const TALENT_BRANCHES=[
  {id:'offense', n:'Offense', ico:'⚔️'},
  {id:'defense', n:'Défense', ico:'🛡️'},
  {id:'arcane',  n:'Arcane',  ico:'🔮'},
  {id:'fortune', n:'Fortune', ico:'🍀'},
  {id:'lignee',  n:'Lignée',  ico:'⭐'},
];
const TALENTS=[
  {id:'t_force', br:'offense', n:'Force brute',        tier:1, max:5, eff:{atkPpct:0.04}, desc:'+4% dégâts physiques / rang'},
  {id:'t_focus', br:'offense', n:'Maîtrise arcanique', tier:1, max:5, eff:{atkMpct:0.04}, desc:'+4% dégâts magiques / rang'},
  {id:'t_prec',  br:'offense', n:'Précision',          tier:4, max:5, eff:{critFlat:0.02}, desc:'+2% critique / rang'},
  {id:'t_cruel', br:'offense', n:'Cruauté',            tier:8, max:3, eff:{critDmgPct:0.12}, desc:'+12% dégâts critiques / rang'},
  {id:'t_const', br:'defense', n:'Constitution',       tier:1, max:5, eff:{hpPct:0.05}, desc:'+5% PV max / rang'},
  {id:'t_armor', br:'defense', n:'Cuir épais',         tier:4, max:5, eff:{mitigFlat:0.02}, desc:'+2% réduction de dégâts / rang'},
  {id:'t_leech', br:'defense', n:'Soif de sang',       tier:8, max:3, eff:{lsFlat:0.02}, desc:'+2% vol de vie / rang'},
  {id:'t_mana',  br:'arcane',  n:'Réserve de mana',    tier:1, max:5, eff:{manaPct:0.06}, desc:'+6% mana max / rang'},
  {id:'t_heal',  br:'arcane',  n:'Mains guérisseuses', tier:4, max:5, eff:{healPct:0.06}, desc:'+6% soins prodigués / rang'},
  {id:'t_swift', br:'arcane',  n:'Célérité',           tier:4, max:5, eff:{spdPct:0.03}, desc:'+3% vitesse / rang'},
  {id:'t_luck',  br:'fortune', n:'Chanceux',           tier:1, max:5, eff:{dropPct:0.04}, desc:'+4% butin / rang'},
  {id:'t_dodge', br:'fortune', n:'Insaisissable',      tier:4, max:5, eff:{dodgeFlat:0.015}, desc:'+1,5% esquive / rang'},
  {id:'t_greed', br:'fortune', n:'Cupidité',           tier:6, max:5, eff:{goldPct:0.08}, desc:'+8% or & essence / rang'},
];
// capstone de "Lignée" — propre à la race (via son pid)
...
function lineageNode(){ let lt=LINEAGE_TALENT[RACES[S.race].pid]||LINEAGE_TALENT.xp;
  return {id:'t_lignee', br:'lignee', n:lt.n, tier:10, max:1, eff:lt.eff, desc:lt.desc, lineage:true}; }
function allTalentNodes(){ return TALENTS.concat([lineageNode()]); }
...
function talentBonus(){
  ...
  let T=S.talents||{};
  allTalentNodes().forEach(node=>{ let r=T[node.id]||0; if(!r)return;
    for(let k in node.eff) b[k]=(b[k]||0)+node.eff[k]*r; });
  return b;
}
...
function addStatus(c,id,turns){ let e=c.statuses.find(s=>s.id===id); if(e)e.turns=Math.max(e.turns,turns); else c.statuses.push({id,turns});
  ...
  if((id==='gel'||id==='paralysie') && c.charging){ c.charging=false; removeStatus(c,'charge');
    ...
    logB(`✋ ${c.ico||''} ${c.name} est interrompu — sa charge est annulée !`); flyText(unitEl(c),'Interrompu !','heal'); }
}
...
function enemyAct(e){
  ...
  if(ab.includes('charge') && !e.charging && !e.chargedRecently && Math.random()<(e.isBoss?0.32:0.4)){
    ...
    flash('#ffcf5a',.18); renderBattle(); sfx('ehit'); endTurn(520); return;
  }
  ...
  if(B.menu==='tech'){
    let rows=cur.moves.map(id=>{ let t=TECH_BY_ID[id]; if(!t)return''; let cd=techCdLeft(cur,t);
      ...
      return `<div class="tech-row ${aff?'':'dim'}" onclick="${aff?`cmdUseTech('${id}')`:''}">
        ...
        <span class="tn">${kindIco}${t.n} ${chips}${effPill}<small>${t.desc}</small></span>
        ${right}</div>`;
    }).join('');
...
function renderHeroScreen(){
  ...
  html+=`<div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
      <span class="cz" style="font-size:14px;color:var(--ember-bright)">⭐ Talents</span>
  ...
  TALENT_BRANCHES.forEach(br=>{
    let nodes = br.id==='lignee' ? [lineageNode()] : TALENTS.filter(t=>t.br===br.id);
    html+=`<div class="tal-branch"><div class="tal-bh">${br.ico} ${br.n}</div>`;
    nodes.forEach(t=>{ let r=talentRank(t.id); let maxed=r>=t.max;
      let lvlOk=S.level>=t.tier; let can=!maxed&&lvlOk&&(S.talentPoints||0)>0;
      html+=`<div class="tal-node ${r>0?'on':''} ${(!lvlOk)?'locked':''}">
        <div class="tal-mid"><div class="tal-n">${t.n} <span class="tal-rank">${r}/${t.max}</span></div>
          <div class="tal-d">${t.desc}${lvlOk?'':` · <span style="color:var(--blood)">niv. ${t.tier} requis</span>`}</div></div>
        <button class="plusbtn" onclick="learnTalent('${t.id}')" ${can?'':'disabled'}>+</button>
      </div>`;
    });
    html+=`</div>`;
  });
  html+=`<button class="btn sm ghost block" style="margin-top:8px" onclick="resetTalents()">↺ Réinitialiser les talents (rend les points)</button>
  </div>`;
  // Anneau & corruption
...
window.learnTalent=function(id){ if((S.talentPoints||0)<=0){toast('Aucun point de talent.');return;}
  let node=allTalentNodes().find(t=>t.id===id); if(!node)return;
  if(S.level<node.tier){toast(`Niveau ${node.tier} requis.`);return;}
  let r=talentRank(id); if(r>=node.max){toast('Talent au maximum.');return;}
