...
/* ÉVOLUTIONS DE RACE — forme avancée (stats accrues + bonus "évolué") débloquée par niveau + essence */
const EVOLUTIONS={
  humain:{key:'dunedain',n:'Dúnedain',ico:'👑',req:18,cost:25,passive:'Lignée des Rois — +12% XP, puissance accrue',pid:'xp',addTree:'tenebres'},
  elfe:{key:'haut_elfe',n:'Haut-Elfe',ico:'🧝‍♀️',req:18,cost:25,passive:'Sang Ancien — +18% esquive, puissance accrue',pid:'dodge',addTree:'foudre'},
  nain:{key:'roi_nain',n:'Seigneur Nain',ico:'⛏️',req:18,cost:25,passive:'Forge-Cœur — +20% PV, puissance accrue',pid:'hp',addTree:'foudre'},
  hobbit:{key:'porteur',n:'Porteur de l\'Anneau',ico:'💍',req:18,cost:25,passive:'Volonté de Fer — +30% butin, puissance accrue',pid:'drop',addTree:'tenebres'},
  orque:{key:'uruk',n:'Uruk-Hai',ico:'👹',req:18,cost:25,passive:'Né pour Tuer — +15% dégâts phys, puissance accrue',pid:'phys',addTree:'feu'},
  rohirrim:{key:'marechal',n:'Maréchal de la Marche',ico:'🐎',req:18,cost:25,passive:'Cavalier d\'Élite — +15% vitesse, puissance accrue',pid:'spd',addTree:'lumiere'},
  istari:{key:'mage_blanc',n:'Mage Blanc',ico:'🧙‍♂️',req:18,cost:25,passive:'Ordre Supérieur — +20% Mana, puissance accrue',pid:'mana',addTree:'tenebres'},
  sylvain:{key:'gardien_sylve',n:'Gardien de la Sylve',ico:'🌳',req:18,cost:25,passive:'Bénédiction d\'Yavanna — +20% soins, puissance accrue',pid:'heal',addTree:'lumiere'},
  haradrim:{key:'seigneur_serpent',n:'Seigneur Serpent',ico:'🐍',req:18,cost:25,passive:'Venin Royal — +10% critique, puissance accrue',pid:'crit',addTree:'givre'},
};
// génère les races évoluées (base +3 partout, arbre supplémentaire, flag "evolved")
Object.entries(EVOLUTIONS).forEach(([from,ev])=>{
  let b=RACES[from]; let nb={}; STAT_KEYS_FOR_EVO(b.base).forEach(k=>nb[k]=b.base[k]+3);
  let trees=b.trees.slice(); if(ev.addTree&&!trees.includes(ev.addTree))trees.push(ev.addTree);
  RACES[ev.key]={n:ev.n,ico:ev.ico,base:nb,trees,passive:ev.passive,pid:ev.pid,evolved:true,evoFrom:from};
});
function STAT_KEYS_FOR_EVO(o){ return Object.keys(o); }
const BASE_RACE_KEYS=Object.keys(RACES).filter(k=>!RACES[k].evolved);
...
function techEff(t,defType){ return Math.max(...techTypes(t).map(ty=>typeMult(ty,defType))); }
...
const MOBS={
  ...
  golem:{n:'Golem de Pierre',ico:'🗿',type:'physique'},
...
const BOSSES={
  ...
  shelob:{n:'Arachne l\'Insatiable',ico:'🕷️',type:'nature'},
  troll_roi:{n:'Roi des Trolls',ico:'🧌',type:'physique'},
  dragon:{n:'Dragon de Cendres',ico:'🐉',type:'feu'},
  spectre_glace:{n:'Liche de Givre',ico:'❄️',type:'givre'},
  bouche:{n:'Bouche de Sauron',ico:'💀',type:'tenebres'},
  gothmog:{n:'Gothmog le Fléau',ico:'👹',type:'feu'},
};
...
const ZONES=[
  ...
  {id:'z5',n:'Champs du Pelennor',ico:'⚔️',type:'physique',lvl:24,mobs:['uruk','orque','nazgul_mob','golem'],boss:'roi_spectre',
  ...
  {id:'z6',n:'Mordor — le Mont Destin',ico:'🌋',type:'tenebres',lvl:34,mobs:['nazgul_mob','spectre','balrog_spawn','demon'],boss:'sauron',
  ...
  {id:'z7',n:'Tour de Cirith Ungol',ico:'🗼',type:'tenebres',lvl:46,mobs:['araignee','chauvesouris','spectre','serpent'],boss:'shelob',
  ...
  {id:'z8',n:'Barad-dûr',ico:'🏯',type:'tenebres',lvl:60,mobs:['nazgul_mob','demon','oeil_volant','golem'],boss:'gothmog',
...
function defaultState(){
  return {
    ...
    autoBattle:false, musicOn:true, sfxOn:true,
...
function derivedOf(kind,id){
  ...
  // ==== NAZGÛL : transformation bien plus marquée ====
  if(kind==='hero'&&S.isNazgul){
    atkM*=1.55; atkP*=1.35; hp*=1.30; mana*=1.25;
    crit+=0.12; spd*=1.12; ls+=5; dodge+=0.05;
  }
  ...
  return {F,E,V,A,C,maxHp:Math.floor(hp),maxMana:Math.floor(mana),atkP:Math.floor(atkP),atkM:Math.floor(atkM),
    spd:Math.floor(spd),crit:clamp(crit,0,0.6),dodge:clamp(dodge,0,0.4),drop,healMul,lifesteal:ls/100,type};
}
function heroDerived(){ return derivedOf('hero',null); }
function allyLevel(id){ return ensureAlly(id).level; }
function allyDerived(id){ let d=derivedOf('ally',id); d.moves=ALLY_BY_ID[id].moves; return d; }
...
function buildPartyCombatant(slot){
  if(slot==='hero'){
    ...
    return {ref:slot,isHero:false,isAlly:true,name:a.n,ico:a.ico,type:d.type,
      ...
      maxHp:d.maxHp,maxMana:d.maxMana,d:{atkP:d.atkP,atkM:d.atkM,crit:d.crit,lifesteal:d.lifesteal,dodge:d.dodge},
      spd:d.spd,statuses:[],alive:ad.hpCur>0,moves:d.moves};
  }
}
function mkMob(mk,lv,diff){ diff=diff||1; let m=MOBS[mk];
  let hp=Math.floor((50+lv*18)*diff);
  return {ref:'',name:m.n,ico:m.ico,type:m.type,hp,maxHp:hp,
    atk:Math.floor((8+lv*2.2)*diff),spd:92+lv+ri(0,16),crit:0.08,statuses:[],alive:true,lv}; }
function mkBoss(bk,lv,diff,opts){ diff=diff||1; opts=opts||{}; let bd=BOSSES[bk];
  let hp=Math.floor((220+lv*42)*diff*(opts.hpMul||1));
  let e={ref:'boss',name:bd.n,ico:bd.ico,type:bd.type,hp,maxHp:hp,
    atk:Math.floor((14+lv*2.8)*diff*(opts.atkMul||1)),spd:100+lv,crit:0.12,statuses:[],alive:true,isBoss:true,lv};
...
function worldDiff(zoneLvl){
  let d = 1.05 + Math.pow(Math.min(zoneLvl,60)/60, 1.5) * 1.35;
  if(S&&S.isNazgul) d *= 1.12;
  return d;
}
function buildEnemies(zone,boss){
  // explore : packs de monstres (plus durs, jusqu'à 4)
  let arr=[];
  let wd=worldDiff(zone.lvl);
...
function beginBattle(enemies,ctx){
  ctx=ctx||{};
  let party=S.party.map(buildPartyCombatant);
  B={party,enemies,all:[],order:[],turnPtr:0,round:1,over:false,result:null,
     ...
     zone:ctx.zone||ZONE_BY_ID[S.curZone],minimized:false,ctx};
...
function dungeonStep(){
  ...
  if(s===RUN.total){
    ...
    let phases=[
      {name:'Forme I'},
      {name:'Forme II — Courroux', atkMul:1.3, hpMul:0.85},
      {name:'Forme III — Déchaînement', atkMul:1.55, hpMul:0.8, enrage:true},
      {name:'Forme IV — Apothéose', atkMul:1.95, hpMul:0.8, enrage:true},
    ];
...
function advancePhase(boss){
  ...
  let nm=Math.floor(boss.maxHp*(ph.hpMul||1));
  boss.maxHp=nm; boss.hp=nm; boss.alive=true;
  boss.atk=Math.floor(boss.atk*(ph.atkMul||1));
...
function dmgCalc(attacker,target,raw,type,opts){
  ...
  if(hasStatus(attacker,'rage'))d*=1.4;
  ...
  if(dg&&Math.random()<dg){ return {dmg:0,eff,crit:false,dodged:true}; }
...
window.cmdItem=function(kind){ if(!canAct())return; let m=B.curMember;
  if(kind==='potion'){ let heal=Math.floor(m.maxHp*0.45); m.hp=Math.min(m.maxHp,m.hp+heal); flyText(unitEl(m),'+'+fmt(heal),'heal'); spawnBurst(unitEl(m),'lumiere',true); logB(`${m.name} boit une Potion (+${fmt(heal)} PV).`); sfx('heal'); }
  else if(kind==='ether'){ let mn=Math.floor(m.maxMana*0.5); m.mana=Math.min(m.maxMana,m.mana+mn); flyText(unitEl(m),'+'+fmt(mn),'mana'); logB(`${m.name} boit un Éther (+${fmt(mn)} mana).`); sfx('magic'); }
  finishMember(); };
...
window.cmdRing=function(){ if(!canAct())return; let m=B.curMember; if(!m.isHero){ toast('Seul le Porteur peut user de l\'Anneau.'); return; }
  ...
  // léger soin + corruption lente
  m.hp=Math.min(m.maxHp,m.hp+Math.floor(m.maxHp*0.12));
  if(!S.isNazgul){ addCorruption(4); }
  S.ringUses=(S.ringUses||0)+1;
  logB(`💍 ${m.name} brandit l'Anneau Unique — ${fmt(total)} dégâts ténébreux !`);
  sfx('ring');
  finishMember();
};
...
function doMove(actor,t,target){
  ...
  if(t.kind==='heal'){
    ...
    let amt=Math.floor((stat*(t.pow)+actor.maxHp*0.05)*healMul);
    let targets;
    if(t.revive){ let dead=B.party.find(p=>!p.alive); if(dead){ dead.alive=true; dead.hp=Math.floor(dead.maxHp*Math.max(0.4,t.pow)); flyText(unitEl(dead),'Ranimé !','heal'); playTechVfx(t,actor,[dead]); logB(`${actor.ico} ${actor.name} ranime ${dead.name} !`); sfx('heal'); finishMember(); return; }
      targets=[lowestAlly()]; }
    else targets= t.target==='allyAll'?aliveParty():[lowestAlly()];
    ...
    let targets= t.target==='ally'?[lowestAlly()]:aliveParty();
    targets.forEach(tt=>{ addStatus(tt,st,3); });
    playTechVfx(t,actor,targets);
    ...
    let hits=t.hits||1; let totalDealt=0; let big=false;
    playTechVfx(t,actor,targets);
    targets.forEach(tt=>{
      if(!tt)return;
      for(let h=0;h<hits;h++){
        ...
        else{ flyText(unitEl(tt),fmt(r.dmg),r.crit?'crit':''); totalDealt+=r.dmg; if(r.crit)big=true;
...
function autoAct(m){
  ...
  let reviveT=moves.find(t=>t.revive&&(m.mana||0)>=t.cost);
  ...
  let healT=moves.filter(t=>t.kind==='heal'&&!t.revive&&(m.mana||0)>=t.cost).sort((a,b)=>b.pow-a.pow)[0];
  let aoeLow=aliveParty().filter(p=>p.hp<p.maxHp*0.6).length>=2;
  if(healT&&(low||(aoeLow&&healT.target==='allyAll'))){ doMove(m,healT,null); return; }
  // 3) buff utile en début de combat
  let buffT=moves.find(t=>t.kind==='buff'&&(m.mana||0)>=t.cost);
  if(buffT&&B.round<=1&&Math.random()<0.5){ doMove(m,buffT,null); return; }
  ...
  let atk=moves.filter(t=>t.kind==='attack'&&(m.mana||0)>=t.cost);
  let best=atk.sort((a,b)=>(b.pow*techEff(b,target.type))-(a.pow*techEff(a,target.type)))[0]||TECH_BY_ID.ph1;
  doMove(m,best,target);
}

/* IA ennemie — concentre le feu, abuse des statuts, frappe parfois en zone */
function enemyAct(e){
  ...
  // AoE boss
  if(e.isBoss&&Math.random()<(enraged?0.5:0.28)){
    logB(`${e.ico} ${e.name} déchaîne une attaque dévastatrice !`); flash('#7a1030',.35); bigShake();
    targets.forEach(tg=>{ let r=dmgCalc(e,tg,e.atk*1.1,e.type,{ignoreType:false});
  ...
  let raw=e.atk*(special?1.7:1)*(enraged?1.2:1);
...
function grantRewards(zone,boss,opt){
  ...
  let mult=opt.mult||1;
  let gold=Math.floor((boss?60:18)*(1+lv*0.5)*(Math.random()*0.4+0.8)*mult);
...
function renderDock(){
  ...
  if(B.menu==='tech'){
    let rows=cur.moves.map(id=>{ let t=TECH_BY_ID[id]; if(!t)return''; let aff=(cur.mana||0)>=t.cost;
      let eff= t.kind==='attack'&&B.target?techEff(t,B.target.type):1;
      ...
      return `<div class="tech-row ${aff?'':'dim'}" onclick="${aff?`cmdUseTech('${id}')`:''}">
        ...
        <span class="tn">${t.n} ${chips}${mark}<small>${t.desc}</small></span>
    ...
    let ringBtn = cur.isHero ? `<button class="cmd ring" onclick="cmdRing()">💍 ${S.isNazgul?'Pouvoir du Spectre':'Pouvoir de l\'Anneau'}</button>` : '';
    el.innerHTML=`<div class="dock-head"><span class="dt">${cur.ico} ${cur.name}</span>${autoBtn}</div>
      ...
      ${ringBtn?`<div style="margin-top:7px">${ringBtn}</div>`:''}`;
  }
}
...
const Music=(function(){
  let cur=null, started=false, vol=0.55;
  ...
  function play(audio){ if(!audio)return; try{ audio.loop=true; audio.volume=0; let p=audio.play(); if(p&&p.catch)p.catch(()=>{}); fadeTo(audio,vol,800); }catch(e){} }
  return {
    ensure(){ // démarre la musique sur le premier geste utilisateur
      if(!(S&&S.musicOn))return; let want=el(); if(!want)return;
...
function openScreen(name){
  ...
  if(name==='map'){ if(!PATH)mapView='map'; renderMap(); }
...
function renderMap(){
  ...
  if(!reached){
    html+=`<button class="btn gold block" onclick="startPath('${z.id}')">🥾 Rejoindre ${z.n} (sentier d'approche)</button>
      <div class="sub" style="text-align:center;font-size:11px;margin-top:6px">Traverse un sentier semé d'embranchements et d'events (combats, trésors cachés, embuscades…) avant d'atteindre la région et de pouvoir y farmer.</div>`;
  } else {
    html+=`<div style="display:flex;gap:8px">
      <button class="btn gold block" onclick="explore()">⚔️ Explorer (combat)</button>
      <button class="btn block ${bossDone?'ghost':'gold'}" onclick="fightBoss()">${BOSSES[z.boss].ico} Donjon${bossDone?' (refaire)':''}</button>
    </div>
    <div class="sub" style="text-align:center;font-size:11px;margin-top:6px">Le donjon : 20 étapes, 4 gardiens, un boss final à 4 formes. Conquiers-le pour débloquer la zone suivante.</div>`;
  }
  ...
  html+=`<div class="card">
    ...
    <div class="sub" style="text-align:center;font-size:11px;margin-top:6px">L'entraînement convertit ton or en XP. Le marchand vend de l'équipement (stock renouvelable).</div>
  </div>`;
  el.innerHTML=html;
}
...
window.explore=function(){ if(!S.reached.includes(S.curZone)){toast('🥾 Rejoins d\'abord la région.');return;} ac(); Music.ensure(); startBattle(false); };
...
window.rest=function(){ if(S.gold<10){toast('Pas assez d\'or.');return;} S.gold-=10; healTeamFull(); sfx('heal'); toast('🏕️ Repos pris. Toute l\'équipe est restaurée.'); renderMap(); renderTop(); save(); };

/* ===================================================================
   CAMP : ENTRAÎNEMENT (or → XP) + MARCHAND (boutique)
   =================================================================== */
function trainingCost(){ return Math.floor(80 + S.level*35); }
function gainHeroXp(xp){ S.xp+=xp; let g=0; while(S.xp>=xpNeeded(S.level)){ S.xp-=xpNeeded(S.level); S.level++; S.statPoints+=3; g++; } return g; }
window.train=function(){ let c=trainingCost(); if(S.gold<c){toast('🪙 Pas assez d\'or.');return;}
  S.gold-=c; let xp=Math.floor(xpNeeded(S.level)*0.55)+20; let g=gainHeroXp(xp);
  sfx(g?'level':'click'); toast(g?`⭐ Entraînement intense — niveau ${S.level} !`:`🏋️ +${fmt(xp)} XP gagnés.`);
...
function ensureShop(refresh){
  ...
  for(let i=0;i<n;i++){
    ...
    it.price=Math.floor(sellPrice(it)*3.2);
    stock.push(it);
  }
  SHOP={stock};
}
...
function renderShop(el){
  ...
  html+=`<div style="display:flex;gap:8px;margin-top:12px">
    <button class="btn block ${S.gold>=rcost?'gold':'ghost'}" onclick="refreshShop()">🔄 Renouveler le stock (🪙 ${fmt(rcost)})</button>
...
/* ===================================================================
   SENTIER D'APPROCHE — chemin à embranchements avec events
   =================================================================== */
const PATH_EVENTS={
  combat:{ico:'⚔️',n:'Embuscade',desc:'Un groupe ennemi barre la route.'},
  elite:{ico:'💀',n:'Sentinelle d\'élite',desc:'Un adversaire redoutable. Gros butin.'},
  treasure:{ico:'💰',n:'Trésor caché',desc:'Un coffre dissimulé hors des sentiers.'},
  camp:{ico:'🏕️',n:'Campement',desc:'Un répit : soigne toute l\'équipe.'},
  merchant:{ico:'🪙',n:'Colporteur',desc:'Une trouvaille : or et essence.'},
  recruit:{ico:'🤝',n:'Rencontre',desc:'Une silhouette amicale au détour du chemin.'},
};
function makePathNode(zone,allowRecruit){
  let pool=['combat','combat','treasure','camp','merchant','elite'];
  if(allowRecruit && (zone.recruits||[]).some(id=>!S.recruited.includes(id))) pool.push('recruit','recruit');
  let type=pick(pool);
  return {type, ...PATH_EVENTS[type], done:false};
}
window.startPath=function(zoneId){
  let zone=ZONE_BY_ID[zoneId]; S.curZone=zoneId;
  let nSteps=4+Math.floor(zone.lvl/20); // 4 à 7 paliers selon la zone
  let steps=[];
  for(let i=0;i<nSteps;i++){
    // chaque palier propose 2 routes (embranchement)
    steps.push([ makePathNode(zone, i>=1), makePathNode(zone, i>=1) ]);
  }
  PATH={zoneId, steps, idx:0, inCombat:false, pendingType:null};
  ac(); sfx('click'); openScreen('map'); renderMap();
};
function renderPath(el){
  let zone=ZONE_BY_ID[PATH.zoneId];
  let html=`<div class="panel-title">🥾 Sentier vers ${zone.n}</div>
    <div class="sub" style="text-align:center;margin-bottom:12px">Palier ${PATH.idx+1} / ${PATH.steps.length} — choisis ta route. Chaque chemin cache un event.</div>
    <div id="mapWrap">`;
  PATH.steps.forEach((pair,i)=>{
    let state = i<PATH.idx?'done':(i===PATH.idx?'current':'locked');
    if(state==='done'){
      let c=pair.find(n=>n.chosen)||pair[0];
      html+=`<div class="zone-node"><div class="zico">${c.ico}</div><div class="zmid"><div class="zname">Palier ${i+1} — ${c.n} ✓</div><div class="zdesc">Franchi.</div></div></div>`;
    } else if(state==='current'){
      html+=`<div class="divider">Palier ${i+1} — deux routes</div>`;
      pair.forEach((n,side)=>{
        html+=`<div class="zone-node current" style="cursor:pointer" onclick="pathChoose(${side})">
          <div class="zico">${n.ico}</div>
          <div class="zmid"><div class="zname">${n.n}</div><div class="zdesc">${n.desc}</div></div>
        </div>`;
      });
    } else {
      html+=`<div class="zone-node locked"><div class="zico">❔</div><div class="zmid"><div class="zname">Palier ${i+1}</div><div class="zdesc">Route inconnue.</div></div></div>`;
    }
    if(i<PATH.steps.length-1)html+=`<div class="zconnector"></div>`;
  });
  html+=`<div class="zconnector"></div>
    <div class="zone-node ${PATH.idx>=PATH.steps.length?'current':'locked'}"><div class="zico">${zone.ico}</div>
      <div class="zmid"><div class="zname">Arrivée — ${zone.n}</div><div class="zdesc">La région : farm débloqué.</div></div></div>`;
  html+=`</div>`;
  html+=`<button class="btn ghost block" style="margin-top:10px" onclick="abandonPath()">↩︎ Abandonner le sentier</button>`;
  el.innerHTML=html;
}
window.abandonPath=function(){ PATH=null; mapView='map'; sfx('click'); renderMap(); renderTop(); };
window.pathChoose=function(side){
  if(!PATH)return; let node=PATH.steps[PATH.idx][side]; node.chosen=true; PATH.pendingType=node.type;
  let zone=ZONE_BY_ID[PATH.zoneId];
  ac();
  if(node.type==='combat'||node.type==='elite'){
    ...
    let enemies;
    if(node.type==='elite'){
      let lv=zone.lvl+ri(1,4); let bk=pick(DUNGEON_MINIBOSS);
      let e=mkBoss(bk,lv,worldDiff(zone.lvl)*1.05,{hpMul:0.7}); e.ref='boss'; enemies=[e];
    } else { enemies=buildEnemies(zone,false); }
    Music.ensure();
    beginBattle(enemies,{label:node.type==='elite'?'Sentinelle d\'élite':'Embuscade',zone,boss:node.type==='elite'});
    return;
  }
  // events non-combat : résolus immédiatement
  if(node.type==='treasure'){
    let lv=zone.lvl+ri(0,4); let r=rollRarity(false,heroDerived().drop,0.10);
    let baseId = r>=5?pick(UNIQUE_BASE_IDS):pickDropBase(zone);
    ...
    healTeamFull(); sfx('heal'); toast('🏕️ Campement : l\'équipe est restaurée.');
  } else if(node.type==='merchant'){
    let g=Math.floor(zone.lvl*18*(Math.random()*0.5+0.8)); let e=ri(2,6); S.gold+=g; S.ess+=e;
    sfx('level'); toast(`🪙 Trouvaille : +${fmt(g)} or, +${e} essence.`);
    ...
    else { let g=Math.floor(zone.lvl*10); S.gold+=g; toast(`Personne… mais +${fmt(g)} or trouvés.`); }
  }
  pathAdvance();
};
function pathCombatWin(){
  let zone=ZONE_BY_ID[PATH.zoneId];
  let elite=PATH.pendingType==='elite';
...
function pathAdvance(){
  if(!PATH)return;
  PATH.idx++;
  save();
  if(PATH.idx>=PATH.steps.length){
    // ARRIVÉE : région rejointe, farm débloqué
    let zone=ZONE_BY_ID[PATH.zoneId];
    if(!S.reached.includes(zone.id))S.reached.push(zone.id);
    PATH=null; mapView='map'; sfx('win');
    toast(`🏁 Tu atteins ${zone.n} ! Exploration et donjon débloqués.`);
    openScreen('map'); renderMap(); renderTop(); save();
    return;
  }
  renderMap();
}
...
function renderTech(){
  ...
  let trees=availableTrees();
  if(!techTab||!trees.includes(techTab))techTab=trees[0];
  let html=`<div class="panel-title">Grimoire de ${RACES[S.race].n}</div>
    <div class="sub" style="text-align:center;margin-bottom:6px">Apprends des techniques (💎 essence) selon ta race. L'Anneau t'ouvre les Ténèbres. Équipe-en jusqu'à 6.</div>
  ...
  let list = techTab==='double'
    ...
    : TECH.filter(t=>!t.types && t.type===techTab && (!t.nazgul||S.isNazgul));
  list.forEach(t=>{
    let learned=S.learned.includes(t.id);
    let equipped=S.equippedTech.includes(t.id);
    let canLearn=S.level>=t.lvl && canLearnTech(t);
    let cost=techCost(t);
    ...
    html+=`<div class="tech-card ${cls}">
      ...
      <div class="tcmid">
        ...
        <div class="tcmeta">${t.desc} · ${t.cost?t.cost+' mana':'sans mana'} · ${t.target==='all'?'tous':t.target==='allyAll'?'équipe':t.target==='ally'?'allié':'cible'}</div>
        <div class="tcmeta" style="color:var(--faint)">Requiert niv. ${t.lvl}${!canLearnTech(t)?' · arbre non maîtrisé':''}</div>
      </div>
      <div style="display:flex;flex-direction:column;gap:4px">
        ${learned
...
window.learnTech=function(id){ let t=TECH_BY_ID[id]; if(S.level<t.lvl){toast('Niveau requis non atteint.');return;}
  if(!canLearnTech(t)){toast('Arbre non maîtrisé.');return;}
...
function renderHeroScreen(){
  ...
  // ÉVOLUTION DE RACE
  let evo=EVOLUTIONS[S.race];
  if(race.evolved){
    html+=`<div class="card" style="border:1px solid var(--ember)"><div class="cz" style="font-size:13px;color:var(--ember-bright)">⭐ Forme évoluée</div>
      <div class="sub" style="font-size:11px;margin-top:4px">Tu as atteint ta forme avancée : ${race.ico} ${race.n}. Stats de base accrues et puissance majorée.</div></div>`;
  } else if(evo){
    let ok=S.level>=evo.req && S.ess>=evo.cost;
    html+=`<div class="card" style="border:1px solid var(--line2)">
      <div class="cz" style="font-size:13px;color:var(--ember-bright);margin-bottom:4px">⭐ Évolution de lignée</div>
      <div class="sub" style="font-size:11px;margin-bottom:8px">Évolue en ${evo.ico} <b>${evo.n}</b> : +3 à toutes les caractéristiques, +10 % de puissance, un nouvel arbre (${TYPES[evo.addTree].ico} ${TYPES[evo.addTree].n}) et ${evo.passive}.</div>
      <div class="sub" style="font-size:11px;margin-bottom:8px">Requis : niveau ${evo.req} · 💎 ${evo.cost} essence ${ok?'':'<span style="color:var(--blood)">(non rempli)</span>'}</div>
      <button class="btn sm ${ok?'gold':'ghost'} block" ${ok?'':'disabled'} onclick="evolveRace()">⭐ Évoluer (💎 ${evo.cost})</button>
    </div>`;
  }
  // allocation
  html+=`<div class="card">
  ...
  html+=`<div class="card">
    ...
    <div class="kv"><span class="k">🎵 Musique de combat</span><button class="btn sm ${S.musicOn?'gold':'ghost'}" onclick="toggleMusic()">${S.musicOn?'Activée':'Coupée'}</button></div>
    <div class="kv"><span class="k">🔊 Effets sonores</span><button class="btn sm ${S.sfxOn?'gold':'ghost'}" onclick="toggleSfx()">${S.sfxOn?'Activés':'Coupés'}</button></div>
  </div>`;
...
window.evolveRace=function(){
  let evo=EVOLUTIONS[S.race]; if(!evo){toast('Aucune évolution disponible.');return;}
  if(S.level<evo.req){toast(`Niveau ${evo.req} requis.`);return;}
  if(S.ess<evo.cost){toast('💎 Pas assez d\'essence.');return;}
  S.ess-=evo.cost; S.race=evo.key;
  S.hpCur=heroDerived().maxHp; S.manaCur=heroDerived().maxMana;
  sfx('level'); toast(`⭐ Tu évolues en ${RACES[S.race].ico} ${RACES[S.race].n} !`);
