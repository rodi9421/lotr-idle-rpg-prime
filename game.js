...
const TECH_CD={
  fe3:3,fe4:3,gi3:3,gi4:3,gi5:4,fo3:3,fo4:3,na5:4,te3:3,te4:5,te5:4,te6:3,ph8:3,fe5:2,
  lu3:4,lu5:5,lu6:4,lu7:4,
  dx1:3,dx2:3,dx3:2,dx4:3,dx5:2,dx6:2,dx7:3,dx8:4,dx9:3,dx10:3,dx11:3,dx12:3,
  bf1:3,bf2:3,bf3:2,db1:2,db2:3,db3:3,db4:2,ph6:3,lu2:3,na4:2,na5:4,
};
TECH.forEach(t=>{ if(TECH_CD[t.id])t.cd=TECH_CD[t.id]; });
const RING_CD=5; // l'Anneau : ultime à recharge (5 tours)
...
function derivedOf(kind,id){
  ...
  if(kind==='hero'&&S.isNazgul){
    atkM*=1.55; atkP*=1.35; hp*=1.30; mana*=1.6;
...
function dmgCalc(attacker,target,raw,type,opts){
  ...
  if(hasStatus(attacker,'fureur'))d*=1.25;
  if(hasStatus(attacker,'faiblesse'))d*=0.7;
...
function ringCdLeft(){ return B&&B.round<B.ringReady?(B.ringReady-B.round):0; }
...
window.cmdRing=function(){ if(!canAct())return; let m=B.curMember; if(!m.isHero){ toast('Seul le Porteur peut user de l\'Anneau.'); return; }
  if(ringCdLeft()>0){ toast(`💍 L'Anneau se recharge : ${ringCdLeft()} tour(s).`); return; }
  B.awaiting=false;
  ...
  B.ringReady=B.round+RING_CD; // recharge
...
function doMove(actor,t,target){
  B.awaiting=false;
  // coût en mana — et ponction de vie en mode Nazgûl (le pouvoir a un prix)
  if(t.cost){ actor.mana=Math.max(0,actor.mana-t.cost);
    if(S.isNazgul&&actor.isHero){ let hpCost=Math.max(1,Math.floor(actor.maxHp*(t.cost/260)));
      actor.hp=Math.max(1,actor.hp-hpCost); flyText(unitEl(actor),'-'+fmt(hpCost),'crit'); }
  }
  setTechCd(actor,t);
...
function autoAct(m){
  ...
  let usable=t=>(m.mana||0)>=t.cost && techCdLeft(m,t)<=0;
...
function renderDock(){
  ...
  if(B.menu==='tech'){
    let rows=cur.moves.map(id=>{ let t=TECH_BY_ID[id]; if(!t)return''; let cd=techCdLeft(cur,t);
      let aff=(cur.mana||0)>=t.cost && cd<=0;
      ...
      let right = cd>0?`<span class="tc cd">⏳ ${cd}</span>`:`<span class="tc">${t.cost?t.cost+' MP':'—'}</span>`;
      return `<div class="tech-row ${aff?'':'dim'}" onclick="${aff?`cmdUseTech('${id}')`:''}">
        <span class="ti">${t.ico}</span>
        <span class="tn">${kindIco}${t.n} ${chips}${mark}<small>${t.desc}</small></span>
        ${right}</div>`;
    }).join('');
    ...
    let rcd=ringCdLeft();
    let ringBtn = cur.isHero ? `<button class="cmd ring ${rcd>0?'dim':''}" onclick="cmdRing()">💍 ${S.isNazgul?'Pouvoir du Spectre':'Pouvoir de l\'Anneau'}${rcd>0?` — ⏳ ${rcd}`:''}</button>` : '';
