...
function renderMap(){
  ...
  if(!bossDone){
    html+=`<button class="btn gold block" onclick="startPath('${z.id}')">🗺️ Carte d'aventure — jusqu'au boss</button>
      <div class="sub" style="text-align:center;font-size:11px;margin-top:6px">Trace ta route à travers la région : embranchements, events, élites et trésors mènent au ${BOSSES[z.boss].ico} ${BOSSES[z.boss].n}. Le vaincre débloque la zone suivante.</div>`;
  } else {
    html+=`<div style="display:flex;gap:8px">
      <button class="btn gold block" onclick="explore()">⚔️ Explorer (farm)</button>
      <button class="btn gold block" onclick="startPath('${z.id}')">🗺️ Refaire la carte</button>
    </div>
    <div class="sub" style="text-align:center;font-size:11px;margin-top:6px">Zone conquise. Farme les combats, ou relance l'aventure pour plus de butin.</div>`;
  }
...
window.explore=function(){ if(!(S.reached.includes(S.curZone)||S.cleared.includes(S.curZone))){toast('🗺️ Termine d\'abord la carte d\'aventure.');return;} ac(); Music.ensure(); startBattle(false); };
